...
/**
 * Siri-style GLSL shaders rendered on a raw WebGL canvas. Self-contained: a
 * single fullscreen triangle drives a fragment shader, no external library.
 * The "fluid-dots" variant is warm-tinted to match the site's Monet palette.
 */
...
// fluid-dots: six metaballs that merge, scatter and gather. Hue biased warm
// (rose → coral → gold, drifting slowly) to echo the Monet dusk palette.
...
vec3 scene(vec2 p, float t){
    ...
    for(int i=0; i<N; i++){
        ...
        // Warm sunset hue: oscillate between rose (~0.94) and gold (~0.11).
        float hue = fract(0.02 + 0.09*sin(t*0.14 + fi*1.1));
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      // WebGL unavailable / shader failure — the loader still shows its caption.
    }
